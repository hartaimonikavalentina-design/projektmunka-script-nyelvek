...
# ---------------------------------------------------------
# ZIP → CSV → DataFrame beolvasás (AUTOMATIKUS CSV név felismerés)
# ---------------------------------------------------------
@st.cache_data
def load_dataframe():
    with zipfile.ZipFile(ZIP_PATH) as z:

        # 1) Kiolvassuk a ZIP tartalmát
        files = z.namelist()
        st.write("ZIP tartalma:", files)

        # 2) Megkeressük a CSV-t automatikusan
        csv_files = [f for f in files if f.lower().endswith(".csv")]
        if not csv_files:
            raise FileNotFoundError("A ZIP-ben nem található CSV fájl.")

        csv_name = csv_files[0]   # első CSV fájl
        st.write("Felismert CSV fájl:", csv_name)

        # 3) Beolvasás a helyes fejléc-sorral
        with z.open(csv_name) as f:
            df = pd.read_csv(
                f,
                sep=";",           # pontosvessző szeparátor
