...
CSV_NAME = "adatok_meteorologia.csv"

# ---------------------------------------------------------
# ZIP → CSV → DataFrame beolvasás (FIXED-WIDTH FILE)
# ---------------------------------------------------------
@st.cache_data
def load_dataframe():
    with zipfile.ZipFile(ZIP_PATH) as z:
        with z.open(CSV_NAME) as f:
            df = pd.read_fwf(
                f,
                skiprows=4,        # meta sorok átugrása
                encoding="latin2"
            )
    return df

# ---------------------------------------------------------
# Diagnosztika – mit olvas be valójában a Pandas?
# ---------------------------------------------------------
st.header("Diagnosztika – beolvasott DataFrame")

try:
    df_test = load_dataframe()
    st.write("Oszlopok:", df_test.columns.tolist())
    st.write(df_test.head(20))
except Exception as e:
    st.error("Diagnosztikai hiba:")
    st.exception(e)
