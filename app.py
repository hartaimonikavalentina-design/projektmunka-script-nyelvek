...
# ---------------------------------------------------------
# ZIP → CSV → DataFrame beolvasás (AUTOMATIKUS szeparátor felismerés)
# ---------------------------------------------------------
@st.cache_data
def load_dataframe():
    with zipfile.ZipFile(ZIP_PATH) as z:
        with z.open(CSV_NAME) as f:
            df = pd.read_csv(
                f,
                sep=None,          # automatikus szeparátor felismerés
                engine="python",   # szükséges a sep=None-hoz
                skiprows=4,        # 3 meta sor + 1 üres sor
                encoding="latin2",
                header=0           # fejléc a következő sor
            )
    return df
